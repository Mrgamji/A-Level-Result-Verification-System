...
router.post('/', authenticateToken, requireRole('institution'), [
  ...
  body('certificateType').trim().notEmpty().withMessage('Certificate type is required'),
], async (req, res) => {
  try {
  ...
  try {
    ...
    fs.createReadStream(req.file.path)
      ...
      .on('data', (data) => {
        ...
        const requiredFields = ['fullName', 'department', 'certificateNumber','certificateType', 'yearOfEntry', 'yearOfGraduation', 'classOfDegree'];
